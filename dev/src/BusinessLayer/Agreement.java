...
public class Agreement implements IAgreement {
    ...
    private final HashMap<Integer, ProductSupplier> products; //- products: hashMap<CatalogID: int, product: Product>
    ...
    public Agreement(int supID, double xDisc, String mod, int daysFromOrder) {//,HashMap<Integer, HashMap<Integer, Double>> discountByProductQuantity,HashMap<Integer, ProductSupplier> products
        ...
        this.products = new HashMap<>();
        if (mod.equals("Pickup")) {
            deliveryMods = DeliveryMode.Pickup;
        } else if (mod.equals("DeliveryByDay")) {
            deliveryMods = DeliveryMode.DeliveryByDay;
        } else {
            deliveryMods = DeliveryMode.DeliveryAfterOrder;
        }
    ...
    public double Calculate_cost(ProductSupplier productSupplier, int quantity) {
        ...
        if (products.containsKey(productSupplier.getCatalogID())) {
            ...
            if (newprice != -1) {
                ...
                cost = (productSupplier.getPrice()) * quantity;
            }
    ...
    public void SetExtraDiscount(int ExtraDiscount) {
        if (ExtraDiscount < 0 | ExtraDiscount > 100)
            throw new IllegalArgumentException("Illegal Discount, the Discount need to be between 0-100 %");
